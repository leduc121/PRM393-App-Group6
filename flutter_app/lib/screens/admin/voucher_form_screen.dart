  ...
  Future<void> _save() async {
    ...
    final data = <String, dynamic>{
      ...
      'maxDiscount': _maxDiscountCtrl.text.trim().isEmpty
          ? null
          : int.parse(_maxDiscountCtrl.text.trim()),
      'minOrderValue': _minOrderCtrl.text.trim().isEmpty
          ? 0
          : int.parse(_minOrderCtrl.text.trim()),
    ...
    if (mounted) {
      ...
      if (result.isSuccess) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Lưu voucher thành công')));
        Navigator.pop(context, true);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result.errorMessage ?? 'Lưu voucher thất bại'),
          ),
        );
      }
    }
          ...
          children: [
            ...
            if (_discountType == 'percentage') ...[
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Số tiền giảm tối đa (Tùy chọn)',
                ),
                keyboardType: TextInputType.number,
              ),
            ],
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'Giá trị đơn tối thiểu (VD: 200000)',
              ),
              keyboardType: TextInputType.number,
            ),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(
                labelText: 'Hạng thành viên áp dụng',
              ),
              items: const [
                DropdownMenuItem(
                  value: 'bronze',
                  child: Text('Bronze (Mọi thành viên)'),
                ),
                DropdownMenuItem(
                  value: 'silver',
                  child: Text('Từ Silver trở lên'),
                ),
                DropdownMenuItem(value: 'gold', child: Text('Từ Gold trở lên')),
                DropdownMenuItem(
                  value: 'platinum',
                  child: Text('Chỉ Platinum'),
                ),
              ],
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'Giới hạn số lần dùng chung (Tùy chọn)',
              ),
              keyboardType: TextInputType.number,
            ),
            const SizedBox(height: 12),
            InkWell(
              ...
              child: InputDecorator(
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    color: _expiresAt != null ? Colors.black87 : Colors.black54,
                  ),
                ),
              ),
            ),
            ...
            SwitchListTile(
              ...
              value: _isActive,
              activeThumbColor: SportZoneTheme.primary,
